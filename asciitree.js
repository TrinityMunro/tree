...
module.exports = function draw_tree(
  tree,
  how_to_get_title,
  how_to_get_children
) {
  ...
  var draw_node = function (tree, onLine, position) {
    ...
    if (nodes) {
      ...
      if (nodes.length == 1) {
        ...
        nodePadding = draw_node(
          node,
          onLine + 2,
          position - offset + foundedPadding
        );
        insert(
          "|",
          onLine + 1,
          position + halfOfCurrentTitle + foundedPadding + nodePadding
        );
        ...
        for (var i = 0; i < nodes.length; i++) {
          ...
          if (i == 0) {
            ...
            insert(
              "/",
              onLine + 1,
              position - 1 + foundedPadding + nodePadding
            );
            insert(
              repeat(whitespace, currentTitle.length),
              onLine + 1,
              position + foundedPadding + nodePadding
            );
            padding = foundedPadding + nodePadding;
